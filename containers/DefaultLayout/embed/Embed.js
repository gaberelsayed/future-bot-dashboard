...
const EmbedAuthor = ({ name, url, icon_url }) => {
  ...
  const authorIcon = icon_url ? (<img src={icon_url} alt={icon_url} role='presentation' className='embed-author-icon' />) : null;
...
const EmbedImage = ({ url }) => {
  ...
  return <a href="#/" className='embed-thumbnail embed-thumbnail-rich'><img className='image' role='presentation' src={url} alt={url}/></a>;
};
...
const EmbedFooter = ({ timestamp, text, icon_url }) => {
  ...
  const footerIcon = text && icon_url ? (
    <img src={icon_url} alt={icon_url} className='embed-footer-icon' role='presentation' width='20' height='20' />
  ) : null;
