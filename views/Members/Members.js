...
class Members extends Component {
  ...
  async componentDidMount(){
    const get_data = await this.props.get_data("members")
    if(get_data !== false){
        const members_list = []
        ...
        if (!get_data.data.task_all){
            get_data.data.task_all = {role_all_bots:'',role_all:'',ban:false,axp:false,lxp:false,nxp:false,txp:false,vxp:false,warn:false,vmute:false,tmute:false,pmute:false}
        }else{
            get_data.data.task_all = JSON.parse(get_data.data.task_all)
        }
  ...
  toggle_tab(tabPane,tab) {
    ...
    id_type_text(type_id) {
      if (type_id .toString()=== this.props.selected_guild.id.toString()){
          return "Server";
      }
      else if (this.state.data.members.find(el => {return el.id.toString() === type_id.toString()})){
          const member = this.state.data.members.find(el => {return el.id.toString() === type_id.toString()});
          member.color = member.color.replace("000000","ffffff");
    ...
    this.setState({
      ...
      .then(res => {
        ...
        member.logs.forEach((log,index) => {
            ...
            if (!(log.by in member.types_by && this.id_type_text(log.by) !== null)){
                member.types_by[log.by] = this.id_type_text(log.by)
            }
            if (!(log.to in member.types_to) && this.id_type_text(log.to) !== null){
                member.types_to[log.to] = this.id_type_text(log.to)
            }
        })
        member.warns.forEach((warn,index) => {
            if (!(warn.bywho in member.types_by_warns) && this.id_type_text(warn.bywho) !== null){
  ...
  edit_data(event) {
    ...
    if(event.target.type === "checkbox"){
      new_data.task_all[event.target.id] = event.target.checked;
    } else {
      new_data.task_all[event.target.id] = event.target.value;
    }
  ...
  edit_data_multi(id,value) {
    ...
    const new_value = [];
      if (!value){
          new_data.task_all[id] = '';
      }else{
          new_data.task_all[id] = value.id;
      }
    this.setState({data:new_data});
    this.check_data_save(new_data);
  }
  ...
  check_data_save(new_data){
    let main = JSON.stringify(new_data.task_all)
    let copy = JSON.stringify(this.state.copy_data.task_all)
  ...
  async send_save(type){
    ...
    if(type === "save"){
      var new_data = {...this.state.data}
      new_data.task_all.guild_id = new_data.guild_id;
      if(await this.props.save_data("task_all_save",new_data.task_all) === true){
  ...
  onSortEnd = ({oldIndex, newIndex}) => {
    ...
    member_roles(cell, row, enumObject, index) {
      ...
      <div className="text-center" style={{overflowWrap:'break-word'}}>
            {this.props.selected_guild.roles.filter(role => member.roles.includes(role.id)).map((item, i) =>
                    <span className="badge m-1" style={{borderWidth: 0.2, borderColor: "#"+(item.color !== 0 && (item.color + Math.pow(16, 6)).toString(16).substr(-6).toUpperCase()), borderStyle:'solid'}}>
                        <div className="mr-1" style={{display:"inline",color:"#"+(item.color !== 0 && (item.color + Math.pow(16, 6)).toString(16).substr(-6).toUpperCase())}}>⬤</div>{item.name}
                    </span>
            )}
      </div>
      );
      ...
      <div className="animated fadeIn">
        <Row className="justify-content-center">
          <Col xs="12" sm="12" md="12">
            <Card>
              <CardHeader>
                <i className="fa fa-balance-scale"></i><strong>الأعظاء</strong>
              </CardHeader>
              <CardBody style={{paddingLeft:"0px",paddingRight:"0px"}} className="text-center">
                          ...
                          <FormGroup>
                            <Row>
                                ...
                                <Col xs="8" sm="8" md="8">
                                  ...
                                  {!(this.state.now_member.total_inviter === 0) &&
                                      <InputGroup className="m-0 mb-1 mt-1">
                                        <Input type="text" className="input-sm form-control-sm" value={this.id_type_text(this.state.now_member.total_inviter)}/>
                                      </InputGroup>
                                  }
                                  {this.state.now_member.inviter &&
                                      <>
                ...
                <Button color="success" onClick={this.toggle_all}>عمل جماعي</Button>
				  <Modal isOpen={this.state.model_all} toggle={this.toggle_all} className={this.props.className}>
					<ModalHeader toggle={this.toggle_all}><i className="fa fa-balance-scale"></i> عمل جماعي</ModalHeader>
					<ModalBody>
                        <Row>
                            <Col md={12} className="m-1">
                                <Row className="text-center">
                                    <Col md={4}>
                                        <InputGroupText className="w-100">الميوتات الكتابية</InputGroupText>
                                    </Col>
                                    <Col md={8}>
                                        <AppSwitch className={'mx-1'} variant={'pill'} color={'primary'} outline={'alt'} id="tmute" onChange={(event) => this.edit_data(event)} checked={this.state.data.task_all ? this.state.data.task_all.tmute : false} label dataOn={'\u2713'} dataOff={'\u2715'} />
                                        <FormText className="help-block">سيقوم البوت بأزالة جميع الميوتات الكتابية</FormText>
                                    </Col>
                                </Row>
                            </Col>
                            <Col md={12} className="m-1">
                                <Row className="text-center">
                                    <Col md={4}>
                                        <InputGroupText className="w-100">الميوتات الصوتية</InputGroupText>
                                    </Col>
                                    <Col md={8}>
                                        <AppSwitch className={'mx-1'} variant={'pill'} color={'primary'} outline={'alt'} id="vmute" onChange={(event) => this.edit_data(event)} checked={this.state.data.task_all ? this.state.data.task_all.vmute : false} label dataOn={'\u2713'} dataOff={'\u2715'} />
                                        <FormText className="help-block">سيقوم البوت بأزالة جميع الميوتات الصوتية</FormText>
                                    </Col>
                                </Row>
                            </Col>
                            <Col md={12} className="m-1">
                                <Row className="text-center">
                                    <Col md={4}>
                                        <InputGroupText className="w-100">المسجونين</InputGroupText>
                                    </Col>
                                    <Col md={8}>
                                        <AppSwitch className={'mx-1'} variant={'pill'} color={'primary'} outline={'alt'} id="pmute" onChange={(event) => this.edit_data(event)} checked={this.state.data.task_all ? this.state.data.task_all.pmute : false} label dataOn={'\u2713'} dataOff={'\u2715'} />
                                        <FormText className="help-block">سيقوم البوت بأزالة جميع المساجين</FormText>
                                    </Col>
                                </Row>
                            </Col>
                            <Col md={12} className="m-1">
                                <Row className="text-center">
                                    <Col md={4}>
                                        <InputGroupText className="w-100">التحذيرات</InputGroupText>
                                    </Col>
                                    <Col md={8}>
                                        <AppSwitch className={'mx-1'} variant={'pill'} color={'primary'} outline={'alt'} id="warn" onChange={(event) => this.edit_data(event)} checked={this.state.data.task_all ? this.state.data.task_all.warn : false} label dataOn={'\u2713'} dataOff={'\u2715'} />
                                        <FormText className="help-block">سيقوم البوت بأزالة جميع التحذيرات</FormText>
                                    </Col>
                                </Row>
                            </Col>
                            <Col md={12} className="m-1">
                                <Row className="text-center">
                                    <Col md={4}>
                                        <InputGroupText className="w-100">الخبرة الصوتية</InputGroupText>
                                    </Col>
                                    <Col md={8}>
                                        <AppSwitch className={'mx-1'} variant={'pill'} color={'primary'} outline={'alt'} id="vxp" onChange={(event) => this.edit_data(event)} checked={this.state.data.task_all ? this.state.data.task_all.vxp : false} label dataOn={'\u2713'} dataOff={'\u2715'} />
                                        <FormText className="help-block">سيقوم البوت بتصفير جميع الخبرة الصوتية</FormText>
                                    </Col>
                                </Row>
                            </Col>
                            <Col md={12} className="m-1">
                                <Row className="text-center">
                                    <Col md={4}>
                                        <InputGroupText className="w-100">الخبرة الكتابية</InputGroupText>
                                    </Col>
                                    <Col md={8}>
                                        <AppSwitch className={'mx-1'} variant={'pill'} color={'primary'} outline={'alt'} id="txp" onChange={(event) => this.edit_data(event)} checked={this.state.data.task_all ? this.state.data.task_all.txp : false} label dataOn={'\u2713'} dataOff={'\u2715'} />
                                        <FormText className="help-block">سيقوم البوت بتصفير جميع الخبرة الكتابية</FormText>
                                    </Col>
                                </Row>
                            </Col>
                            <Col md={12} className="m-1">
                                <Row className="text-center">
                                    <Col md={4}>
                                        <InputGroupText className="w-100">الخبرة الحالية</InputGroupText>
                                    </Col>
                                    <Col md={8}>
                                        <AppSwitch className={'mx-1'} variant={'pill'} color={'primary'} outline={'alt'} id="nxp" onChange={(event) => this.edit_data(event)} checked={this.state.data.task_all ? this.state.data.task_all.nxp : false} label dataOn={'\u2713'} dataOff={'\u2715'} />
                                        <FormText className="help-block">سيقوم البوت بتصفير الخبرة الحالية لدى الجميع</FormText>
                                    </Col>
                                </Row>
                            </Col>
                            <Col md={12} className="m-1">
                                <Row className="text-center">
                                    <Col md={4}>
                                        <InputGroupText className="w-100">المستويات</InputGroupText>
                                    </Col>
                                    <Col md={8}>
                                        <AppSwitch className={'mx-1'} variant={'pill'} color={'primary'} outline={'alt'} id="lxp" onChange={(event) => this.edit_data(event)} checked={this.state.data.task_all ? this.state.data.task_all.lxp : false} label dataOn={'\u2713'} dataOff={'\u2715'} />
                                        <FormText className="help-block">سيقوم البوت بتصفير المستويات لدى الجميع</FormText>
                                    </Col>
                                </Row>
                            </Col>
                            <Col md={12} className="m-1">
                                <Row className="text-center">
                                    <Col md={4}>
                                        <InputGroupText className="w-100">مجموع الخبرة</InputGroupText>
                                    </Col>
                                    <Col md={8}>
                                        <AppSwitch className={'mx-1'} variant={'pill'} color={'primary'} outline={'alt'} id="axp" onChange={(event) => this.edit_data(event)} checked={this.state.data.task_all ? this.state.data.task_all.axp : false} label dataOn={'\u2713'} dataOff={'\u2715'} />
                                        <FormText className="help-block">سيقوم البوت بتصفير مجموع الخبرة لدى الجميع</FormText>
                                    </Col>
                                </Row>
                            </Col>
                            <Col md={12} className="m-1">
                                <Row className="text-center">
                                    <Col md={4}>
                                        <InputGroupText className="w-100">الباندات</InputGroupText>
                                    </Col>
                                    <Col md={8}>
                                        <AppSwitch className={'mx-1'} variant={'pill'} color={'primary'} outline={'alt'} id="ban" onChange={(event) => this.edit_data(event)} checked={this.state.data.task_all ? this.state.data.task_all.ban : false} label dataOn={'\u2713'} dataOff={'\u2715'} />
                                        <FormText className="help-block">سيقوم البوت بأزالة جميع باندات السيرفر</FormText>
                                    </Col>
                                </Row>
                            </Col>
                            <Col md={12} className="m-1">
                                <Row className="text-center">
                                    <Col md={4}>
                                        <InputGroupText className="w-100">رول جماعي</InputGroupText>
                                    </Col>
                                    <Col md={8} className="justify-content-center">
                                        <Select
                                          closeMenuOnSelect={true}
                                          components={makeAnimated()}
                                          value={this.state.data.task_all ? this.props.selected_guild.roles.filter(role => role.id === this.state.data.task_all.role_all) : null}
                                          options={this.props.selected_guild.roles}
                                          onChange={(value) => this.edit_data_multi("role_all",value)}
                                          styles={colourStyles}
                                          placeholder="أختر الرول"
                                        />
                                        <FormText className="help-block">سيقوم البوت بأعطاء الرتبة لجميع الأعضاء ماعدا البوتات</FormText>
                                    </Col>
                                </Row>
                            </Col>
                            <Col md={12} className="m-1">
                                <Row className="text-center">
                                    <Col md={4}>
                                        <InputGroupText className="w-100">رول جماعي (بوتات)</InputGroupText>
                                    </Col>
                                    <Col md={8} className="justify-content-center">
                                        <Select
                                          closeMenuOnSelect={true}
                                          components={makeAnimated()}
                                          value={this.state.data.task_all ? this.props.selected_guild.roles.filter(role => role.id === this.state.data.task_all.role_all_bots) : null}
                                          options={this.props.selected_guild.roles}
                                          onChange={(value) => this.edit_data_multi("role_all_bots",value)}
                                          styles={colourStyles}
                                          placeholder="أختر الرول"
                                        />
                                        <FormText className="help-block">سيقوم البوت بأعطاء الرتبة لجميع البوتات</FormText>
                                    </Col>
                                </Row>
                            </Col>
                        </Row>
				...
				<BootstrapTable data={this.state.data.members} version="4" striped hover pagination search bordered={ false } options={{
				  sortIndicator: true,
				  hideSizePerPage: true,
				  paginationSize: 3,
				  hidePageListOnlyOnePage: true,
				  ...
				  withFirstAndLast: false,
				  onRowClick: (row) => {
					  this.load_member(row);
				  }
				}}>
                  <TableHeaderColumn dataAlign="center" width={"5%"} dataField="status" dataFormat={this.member_avatar} dataSort>Status</TableHeaderColumn>
                  <TableHeaderColumn dataAlign="center" isKey width={"20%"} dataField="id" dataFormat={this.member_name} dataSort>Member</TableHeaderColumn>
                  <TableHeaderColumn dataAlign="center" width={"20%"} dataField="created_at" dataFormat={this.member_age} dataSort>Age</TableHeaderColumn>
                  <TableHeaderColumn dataAlign="center" width={"40%"} dataField="total_exp" dataFormat={this.member_xp} dataSort>Activity</TableHeaderColumn>
                  <TableHeaderColumn dataAlign="center" width={"15%"} dataField="total_exp" dataFormat={this.member_roles} dataSort>Roles</TableHeaderColumn>
				</BootstrapTable>
              </CardBody>
            </Card>
          </Col>
