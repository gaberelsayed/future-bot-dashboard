...
class addemoji extends Component {
  ...
  async componentDidMount(){
    const get_data = await this.props.get_data("addemoji_command")
    if(get_data !== false){
      get_data.data.addemoji_commands.forEach((command, index) => {
        get_data.data.addemoji_commands[index] = {};
        get_data.data.addemoji_commands[index].label = command;
        get_data.data.addemoji_commands[index].value = command;
      })
  ...
  async send_save(type){
    ...
    if(type === "save"){
      var new_data = {...this.state.data}
      var new_commands = []
      new_data.addemoji_commands.forEach((command, index) => {
        new_commands.push(command.value)
      })
      new_data.addemoji_commands = new_commands
    ...
    const styles = reactCSS({
      'default': {
        color: {
          height: '14px',
          borderRadius: '2px',
          background: `${this.state.data ? this.state.data.addemoji_log_color.replace("0x","#") : null}`,
        },
        swatch: {
      ...
      <div className="animated fadeIn">
        <Row className="justify-content-center">
          <Col xs="12" sm="6" md="6">
            <Card>
              <CardHeader>
                <i className="fa fa-puzzle-piece"></i><strong>{this.props.language.dashboard.commands.addemoji_command.settings}</strong>
                <div className="card-header-actions">
                  ...
                  <Modal isOpen={this.state.modal_help} toggle={this.toggle_help} className={this.props.className}>
                    <ModalHeader toggle={this.toggle_help}><i className="fa fa-puzzle-piece"></i> {this.props.language.dashboard.commands.addemoji_command.title}</ModalHeader>
                    <ModalBody>
                      <FormGroup>
                        <Label><strong>{this.props.language.help_titles.about_title}</strong></Label>
                        <Label>{this.props.language.dashboard.commands.addemoji_command.help.about_description}</Label>
                        {this.props.language.dashboard.commands.addemoji_command.help.about_example.split('\n').map((item, i) => <FormText key={i} className="help-block mt-0">{item}</FormText>)}
                        <Label className="mb-0 mt-4"><strong>{this.props.language.help_titles.permissions_need}</strong></Label>
                        <Label className="mt-0">{this.props.language.dashboard.commands.addemoji_command.help.permissions_note}</Label>
                        {this.props.language.dashboard.commands.addemoji_command.help.permissions_help.split('\n').map((item, i) =>
                        ...
                        <div id="accordion">
                          <Card>
                            ...
                            <Collapse isOpen={this.state.accordion[0]} data-parent="#accordion" id="collapseOne" aria-labelledby="headingOne">
                              <CardBody>
                                {this.props.language.dashboard.commands.addemoji_command.help.tutorial_text.map((item, i) =>
                                <div key={i}>
                          ...
                          <Card>
                            ...
                            <Collapse isOpen={this.state.accordion[1]} data-parent="#accordion" id="collapseTwo">
                              <CardBody>
                                {this.props.language.dashboard.commands.addemoji_command.help.tutorial_video ? <div><YouTube opts={{width: "100%",height: "100%"}} videoId={this.props.language.dashboard.commands.addemoji_command.help.tutorial_video}/></div>: <div>{this.props.language.help_titles.soon}</div>}
                              </CardBody>
                            </Collapse>
                          </Card>
                        </div>
              ...
              <CardBody>
                <FormGroup>
                  <Label>{this.props.language.dashboard.commands.addemoji_command.addemoji_commands_title}</Label>
                  <Creatable
                    components={makeAnimated()}
                    value={this.state.data.addemoji_commands}
                    onChange={(value) => this.edit_data_multi_commands("addemoji_commands",value)}
                    styles={colourStyles}
                    placeholder={this.props.language.dashboard.commands.addemoji_command.addemoji_commands_placeholder}
                    noOptionsMessage={() => null}
                    isMulti
                  />
                  <FormText className="help-block">{this.props.language.dashboard.commands.addemoji_command.addemoji_commands_example}</FormText>
                </FormGroup>
              </CardBody>
            </Card>
          </Col>
          <Col xs="12" sm="6" md="6">
            <Card>
              <CardHeader>
                <i className="fa fa-puzzle-piece"></i><strong>{this.props.language.dashboard.commands.addemoji_command.addemoji_roles_header}</strong>
              </CardHeader>
              <CardBody>
                <FormGroup>
                  <Label>{this.props.language.dashboard.commands.addemoji_command.addemoji_roles_title}</Label>
                  <Select
                    closeMenuOnSelect={false}
                    components={makeAnimated()}
                    value={this.props.selected_guild.roles.filter(role => this.state.data.addemoji_roles.includes(role.id))}
                    options={this.props.selected_guild.roles}
                    onChange={(value) => this.edit_data_multi("addemoji_roles",value)}
                    styles={colourStyles}
                    placeholder={this.props.language.dashboard.commands.addemoji_command.addemoji_roles_placeholder}
                    isMulti
                  />
                  <FormText className="help-block">{this.props.language.dashboard.commands.addemoji_command.addemoji_roles_example}</FormText>
                </FormGroup>
              </CardBody>
            </Card>
          </Col>
          <Col xs="12" sm="6" md="6">
            <Card>
              <CardHeader>
                <i className="fa fa-puzzle-piece"></i><strong>{this.props.language.dashboard.commands.addemoji_command.addemoji_log_title}</strong>
              </CardHeader>
              <CardBody>
                <FormGroup>
                  ...
                  </div>
                  {
                    ...
                    <div style={ styles.cover } onClick={ this.handleCloseColor }/>
                      <SketchPicker color={this.state.data.addemoji_log_color ? this.state.data.addemoji_log_color.replace("0x","#") : "#ffffff"} onChange={(event) => this.edit_data_color("addemoji_log_color",event)} disableAlpha={true} />
                    </div> : null
                  }
                </FormGroup>
                <FormGroup>
                  <Label>{this.props.language.dashboard.commands.addemoji_command.addemoji_log_title_title}</Label>
                  <Input type="text" id="addemoji_log_title" onChange={(event) => this.edit_data(event)} value={this.state.data.addemoji_log_title || ''} placeholder={this.props.language.dashboard.commands.addemoji_command.addemoji_log_title_placeholder}/>
                  <FormText className="help-block">{this.props.language.dashboard.commands.addemoji_command.addemoji_log_title_example}</FormText>
                </FormGroup>
                <FormGroup>
                  <Label>{this.props.language.dashboard.commands.addemoji_command.addemoji_log_channel_title}</Label>
                  <Input type="select" id="addemoji_log_channel" value={this.state.data.addemoji_log_channel} onChange={(event) => this.edit_data(event)}>
                    <option value="">{this.props.language.dashboard.commands.addemoji_command.addemoji_log_channel_disable}</option>
                  ...
                  <FormText className="help-block">{this.props.language.dashboard.commands.addemoji_command.addemoji_log_channel_example}</FormText>
                </FormGroup>
              </CardBody>
            </Card>
          </Col>
