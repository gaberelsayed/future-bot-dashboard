...
class Embed extends Component {
  ...
  async componentDidMount(){
    const get_data = await this.props.get_data("embed_command")
    if(get_data !== false){
      get_data.data.embed_commands.forEach((command, index) => {
        get_data.data.embed_commands[index] = {};
        get_data.data.embed_commands[index].label = command;
        get_data.data.embed_commands[index].value = command;
      })
  ...
  async send_save(type){
    ...
    if(type === "save"){
      var new_data = {...this.state.data}
      var new_commands = []
      new_data.embed_commands.forEach((command, index) => {
        new_commands.push(command.value)
      })
      new_data.embed_commands = new_commands
    ...
    const styles = reactCSS({
      'default': {
        color: {
          height: '14px',
          borderRadius: '2px',
          background: `${this.state.data ? this.state.data.embed_log_color.replace("0x","#") : null}`,
        },
        swatch: {
      ...
      <div className="animated fadeIn">
        <Row className="justify-content-center">
          <Col xs="12" sm="6" md="6">
            <Card>
              <CardHeader>
                <i className="fa fa-i-cursor"></i><strong>{this.props.language.dashboard.commands.embed_command.settings}</strong>
                <div className="card-header-actions">
                  ...
                  <Modal isOpen={this.state.modal_help} toggle={this.toggle_help} className={this.props.className}>
                    <ModalHeader toggle={this.toggle_help}><i className="fa fa-i-cursor"></i> {this.props.language.dashboard.commands.embed_command.title}</ModalHeader>
                    <ModalBody>
                      <FormGroup>
                        <Label><strong>{this.props.language.help_titles.about_title}</strong></Label>
                        <Label>{this.props.language.dashboard.commands.embed_command.help.about_description}</Label>
                        {this.props.language.dashboard.commands.embed_command.help.about_example.split('\n').map((item, i) => <FormText key={i} className="help-block mt-0">{item}</FormText>)}
                        <Label className="mb-0 mt-4"><strong>{this.props.language.help_titles.permissions_need}</strong></Label>
                        <Label className="mt-0">{this.props.language.dashboard.commands.embed_command.help.permissions_note}</Label>
                        {this.props.language.dashboard.commands.embed_command.help.permissions_help.split('\n').map((item, i) =>
                        ...
                        <div id="accordion">
                          <Card>
                            ...
                            <Collapse isOpen={this.state.accordion[0]} data-parent="#accordion" id="collapseOne" aria-labelledby="headingOne">
                              <CardBody>
                                {this.props.language.dashboard.commands.embed_command.help.tutorial_text.map((item, i) =>
                                <div key={i}>
                          ...
                          <Card>
                            ...
                            <Collapse isOpen={this.state.accordion[1]} data-parent="#accordion" id="collapseTwo">
                              <CardBody>
                                {this.props.language.dashboard.commands.embed_command.help.tutorial_video ? <div><YouTube opts={{width: "100%",height: "100%"}} videoId={this.props.language.dashboard.commands.embed_command.help.tutorial_video}/></div>: <div>{this.props.language.help_titles.soon}</div>}
                              </CardBody>
                            </Collapse>
                          </Card>
                        </div>
              ...
              <CardBody>
                <FormGroup>
                  <Label>{this.props.language.dashboard.commands.embed_command.embed_commands_title}</Label>
                  <Creatable
                    components={makeAnimated()}
                    value={this.state.data.embed_commands}
                    onChange={(value) => this.edit_data_multi_commands("embed_commands",value)}
                    styles={colourStyles}
                    placeholder={this.props.language.dashboard.commands.embed_command.embed_commands_placeholder}
                    noOptionsMessage={() => null}
                    isMulti
                  />
                  <FormText className="help-block">{this.props.language.dashboard.commands.embed_command.embed_commands_example}</FormText>
                </FormGroup>
                <FormGroup check className="checkbox">
                  <Input className="mr-1" type="checkbox" color={'info'} id="deny_embed" onChange={(event) => this.edit_data(event)} checked={this.state.data.deny_embed} />
                  <Label check htmlFor="deny_embed">{this.props.language.dashboard.commands.embed_command.deny_embed_title}</Label>
                  <FormText className="help-block">{this.props.language.dashboard.commands.embed_command.deny_embed_example}</FormText>
                </FormGroup>
              </CardBody>
            </Card>
          </Col>
          <Col xs="12" sm="6" md="6">
            <Card>
              <CardHeader>
                <i className="fa fa-i-cursor"></i><strong>{this.props.language.dashboard.commands.embed_command.embed_roles_header}</strong>
              </CardHeader>
              <CardBody>
                <FormGroup>
                  <Label>{this.props.language.dashboard.commands.embed_command.embed_roles_title}</Label>
                  <Select
                    closeMenuOnSelect={false}
                    components={makeAnimated()}
                    value={this.props.selected_guild.roles.filter(role => this.state.data.embed_roles.includes(role.id))}
                    options={this.props.selected_guild.roles}
                    onChange={(value) => this.edit_data_multi("embed_roles",value)}
                    styles={colourStyles}
                    placeholder={this.props.language.dashboard.commands.embed_command.embed_roles_placeholder}
                    isMulti
                  />
                  <FormText className="help-block">{this.props.language.dashboard.commands.embed_command.embed_roles_example}</FormText>
                </FormGroup>
                <FormGroup>
                  <Label>{this.props.language.dashboard.commands.embed_command.embed_deny_channels_title}</Label>
                  <Select
                    closeMenuOnSelect={false}
                    components={makeAnimated()}
                    value={this.props.selected_guild.channels.filter(channel => channel.type === 0).filter(channel => this.state.data.embed_deny_channels.includes(channel.id))}
                    options={this.props.selected_guild.channels.filter(channel => channel.type === 0)}
                    onChange={(value) => this.edit_data_multi("embed_deny_channels",value)}
                    isOptionDisabled={(option) => option.disabled === true}
                    styles={colourStyles}
                    placeholder={this.props.language.dashboard.commands.embed_command.embed_deny_channels_placeholder}
                    isMulti
                  />
                  <FormText className="help-block">{this.props.language.dashboard.commands.embed_command.embed_deny_channels_example}</FormText>
                </FormGroup>
              </CardBody>
            </Card>
          </Col>
          <Col xs="12" sm="6" md="6">
            <Card>
              <CardHeader>
                <i className="fa fa-i-cursor"></i><strong>{this.props.language.dashboard.commands.embed_command.embed_log_title}</strong>
              </CardHeader>
              <CardBody>
                <FormGroup>
                  ...
                  </div>
                  {
                    ...
                    <div style={ styles.cover } onClick={ this.handleCloseColor }/>
                      <SketchPicker color={this.state.data.embed_log_color ? this.state.data.embed_log_color.replace("0x","#") : "#ffffff"} onChange={(event) => this.edit_data_color("embed_log_color",event)} disableAlpha={true} />
                    </div> : null
                  }
                </FormGroup>
                <FormGroup>
                  <Label>{this.props.language.dashboard.commands.embed_command.embed_log_title_title}</Label>
                  <Input type="text" id="embed_log_title" onChange={(event) => this.edit_data(event)} value={this.state.data.embed_log_title || ''} placeholder={this.props.language.dashboard.commands.embed_command.embed_log_title_placeholder}/>
                  <FormText className="help-block">{this.props.language.dashboard.commands.embed_command.embed_log_title_example}</FormText>
                </FormGroup>
                <FormGroup>
                  <Label>{this.props.language.dashboard.commands.embed_command.embed_log_channel_title}</Label>
                  <Input type="select" id="embed_log_channel" value={this.state.data.embed_log_channel} onChange={(event) => this.edit_data(event)}>
                    <option value="">{this.props.language.dashboard.commands.embed_command.embed_log_channel_disable}</option>
                  ...
                  <FormText className="help-block">{this.props.language.dashboard.commands.embed_command.embed_log_channel_example}</FormText>
                </FormGroup>
              </CardBody>
            </Card>
          </Col>
