...
class protlinks extends Component {
  constructor(props) {
    super(props);
    ...
    this.edit_data_list = this.edit_data_list.bind(this);
    this.toggle_help = this.toggle_help.bind(this);
  }
  
  async componentDidMount(){
    const get_data = await this.props.get_data("link_protect")
    if(get_data !== false){
  ...
  add_new_list() {
    if (this.state.data.list.length === 0 || (this.state.data.list[this.state.data.list.length-1].link !== "")){
      var new_data = {...this.state.data}
      new_data.list.push({link : "",channels:[]});
      this.setState({data:new_data});
      this.check_data_save(new_data);
    }
  }

  edit_data_list(type,value,key) {
    var new_data = {...this.state.data}
    if (type === "channels"){
      new_data.list[key].channels = []
      value.forEach((v, i) => {
        new_data.list[key].channels.push(v.id)
      })
    }else {
      new_data.list[key].link = value.target.value;
    }
    this.setState({
      data:new_data,
    });
      ...
      <div className="animated fadeIn">
        <Row className="justify-content-center">
          <Col xs="12" sm="6" md="6">
            <Card>
              <CardHeader>
                <i className="nav-icon icon-shield"></i><strong>{this.props.language.dashboard.protect.anti_links.link_protect_settings}</strong>
              ...
              <CardBody>
                <FormGroup>
                                ...
                                <FormGroup>
                                  <InputGroup>
                                    <InputGroupAddon addonType="prepend">
                                      <InputGroupText>{this.props.language.dashboard.protect.anti_links.link_title}</InputGroupText>
                                    </InputGroupAddon>
                                    <Input type="text" id="link" placeholder={this.props.language.dashboard.protect.anti_links.link_placeholder} onChange={(event) => this.edit_data_list("link",event,index)} value={item.link || ''}/>
                                  ...
                                  <Select
                                    ...
                                    value={this.props.selected_guild.channels.filter(channel => channel.type === 0).filter(channel => item.channels.includes(channel.id))}
                                    options={this.props.selected_guild.channels.filter(channel => channel.type === 0)}
                                    onChange={(value) => this.edit_data_list("channels",value,index)}
                                    styles={colourStyles}
                                    placeholder={this.props.language.dashboard.protect.anti_links.link_channels_placeholder}
                                    isMulti
                                  />
                                </FormGroup>
                              </Col>
                  ...
                  <Row className="justify-content-center">
                    <FormGroup>
                      <Button block color="primary" className="btn-pill" onClick={() => this.add_new_list()}><i className="fa fa-plus"></i></Button>
                    </FormGroup>
                  </Row>
                </FormGroup>
                <FormGroup>
                  <Label>{this.props.language.dashboard.protect.anti_links.allowed_channels_title}</Label>
                  <Select
                    closeMenuOnSelect={false}
                    components={makeAnimated()}
                    ...
                    styles={colourStyles}
                    placeholder={this.props.language.dashboard.protect.anti_links.allowed_channels_placeholder}
                    isMulti
                  />
                  <FormText className="help-block">{this.props.language.dashboard.protect.anti_links.allowed_channels_example}</FormText>
                </FormGroup>
              </CardBody>
            </Card>
          </Col>
          <Col xs="12" sm="6" md="6">
            <Card>
              <CardHeader>
                <i className="nav-icon icon-shield"></i><strong>{this.props.language.dashboard.protect.anti_links.link_log_title}</strong>
              </CardHeader>
              <CardBody>
                <FormGroup>
                ...
                <FormGroup>
                  <Label>{this.props.language.dashboard.protect.anti_links.link_log_title_title}</Label>
                  <Input type="text" id="link_log_title" onChange={(event) => this.edit_data(event)} value={this.state.data.link_log_title || ''} placeholder={this.props.language.dashboard.protect.anti_links.link_log_title_placeholder}/>
                  <FormText className="help-block">{this.props.language.dashboard.protect.anti_links.link_log_title_example}</FormText>
                </FormGroup>
                <FormGroup>
                  <Label>{this.props.language.dashboard.protect.anti_links.link_log_channel_title}</Label>
                  <Input type="select" id="link_log_channel" value={this.state.data.link_log_channel} onChange={(event) => this.edit_data(event)}>
                    <option value="">{this.props.language.dashboard.protect.anti_links.link_log_channel_disable}</option>
                  ...
                  <FormText className="help-block">{this.props.language.dashboard.protect.anti_links.link_log_channel_example}</FormText>
                </FormGroup>
              </CardBody>
            </Card>
          </Col>
